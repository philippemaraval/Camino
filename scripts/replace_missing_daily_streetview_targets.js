...
const GEOJSON_CANDIDATE_PATHS = [
    path.join(ROOT_DIR, 'backend', 'data', 'marseille_rues_light.geojson'),
    path.join(ROOT_DIR, 'backend', 'data', 'marseille_rues_enrichi.geojson'),
    path.join(ROOT_DIR, 'data', 'marseille_rues_light.geojson'),
    path.join(ROOT_DIR, 'data', 'marseille_rues_enrichi.geojson'),
];
...
async function fetchStreetViewMetadata(streetName, geometryIndex, apiKey, timeoutMs) {
    ...
    params.set('location', streetViewGeometry?.location || `${streetName}, Marseille, France`);
